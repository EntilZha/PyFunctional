...
def type_checking() -> None:
    ...
    t_drop_while: Sequence[int] = seq([1, 2, 3, 4, 5, 1, 2]).drop_while(lambda x: x < 3)

    t_take: Sequence[int] = seq([1, 2, 3, 4]).take(2)

    t_take_while: Sequence[int] = seq([1, 2, 3, 4, 5, 1, 2]).take_while(lambda x: x < 3)
    ...
    t_peek: list[int] = seq([1, 2, 3, 4]).peek(print).map(lambda x: x**2).to_list()
    ...
    t_zip_with_index: Sequence[tuple[str, int]] = seq(["a", "b", "c"]).zip_with_index()
    ...
    t_partition: tuple[Sequence[int], Sequence[int]] = seq([-1, 1, -2, 2]).partition(
        lambda x: x < 0
    )
